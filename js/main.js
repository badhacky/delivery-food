...
const buttonAuth = document.querySelector('.button-auth'),
  ...
  restaurantCardInfo = document.querySelector('.restaurant__card-info'),
...
function openProducts(event) {
  ...
  if (login) {
    ...
    if (restaurant) {
      ...
      restaurantCardInfo.classList.remove('hide');
        ...
        productsStrings.forEach(function (productsString) {
          getData(`./db/${productsString}`)
            .then(function (data) {
              ...
              restaurantTitle.textContent = 'Найдено ' + searchGoods.length + ' результатов:';
              restaurantCardInfo.classList.add('hide');
              restaurantStars.textContent = '';
...
// foo(1, 'привет', [1, 2, 3], { a: 1, b: 2 }, function () {
//   console.log(1);
// });
